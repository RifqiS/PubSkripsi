...
for ($i = 2; $i <= $jumlah_baris; $i++) {
	...
	$NIP   			 = $data->val($i, 2);
	$Hari  			 = $data->val($i, 3);
	$Jam_Awal  	   	 = $data->val($i, 4);
	$Jam_Akhir  	 = $data->val($i, 5);
	$Kelas  		 = $data->val($i, 6);
	$Ruang     		 = $data->val($i, 7);
	$Gabungan  		 = $data->val($i, 8);

		if ($Kode_MataKuliah != "" && $NIP != "" && $Hari != "" && $Kelas != "") {
			// input data ke database (table data_pegawai)
			$input = mysqli_query($db, "INSERT INTO jadwal VALUES('','$Kode_MataKuliah','$NIP','$Hari','$Jam_Awal','$Jam_Akhir','$Kelas','$Ruang','$Gabungan')");
			$berhasil++;
		}
}
...
header("location:jadwal.php?berhasil=$berhasil");
?>